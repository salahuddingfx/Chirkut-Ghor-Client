...
import { Star, X, AlertCircle, Sparkles, Heart, Smile, Frown, Meh, CheckCircle } from 'lucide-react';
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 animate-fade-in">
      <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto animate-slide-up">
        ...
        <div className="sticky top-0 bg-gradient-to-r from-maroon to-pink-600 text-white p-6 flex items-center justify-between">
          <h2 className="text-2xl font-black">Share Your Experience <Sparkles className="inline h-5 w-5 mx-1" /></h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          ...
          <div className="space-y-3">
            ...
            {rating > 0 && (
              <p className="text-sm text-slate/60">
                {rating === 5 && <span className="flex items-center gap-1"><Heart className="h-4 w-4 text-red-500 fill-current" /> Absolutely love it!</span>}
                {rating === 4 && <span className="flex items-center gap-1"><Smile className="h-4 w-4 text-yellow-500" /> Very satisfied</span>}
                {rating === 3 && <span className="flex items-center gap-1"><Meh className="h-4 w-4 text-gray-500" /> It's okay</span>}
                {rating === 2 && <span className="flex items-center gap-1"><Frown className="h-4 w-4 text-orange-500" /> Not great</span>}
                {rating === 1 && <span className="flex items-center gap-1"><Frown className="h-4 w-4 text-red-500" /> Poor experience</span>}
              </p>
            )}
          </div>
          ...
          <div className="bg-amber-50 dark:bg-amber-900/20 border-2 border-amber-200 dark:border-amber-800 p-4 rounded-2xl text-sm text-amber-800 dark:text-amber-200">
            <p className="font-bold mb-1"><Sparkles className="inline h-4 w-4 mx-0.5" /> Your review matters!</p>
          ...
          <div className="flex gap-4 pt-6 border-t-2 border-slate/10">
            ...
            >
              {loading ? 'Publishing...' : <span>Publish Review <Sparkles className="inline h-4 w-4 mx-0.5" /></span>}
            </button>
          </div>
        </form>
      </div>
